...
由于 UniPass Wallet 是一款支持多链的智能合约钱包，并且多链地址一致。 在多链账户部署策略上，为了节省用户的手续费，采用**按需部署**策略：

- 初始注册时，只在 AuthChain(Polygon) 上部署智能合约钱包
- 在非 AuthChain 链上使用 UniPass Wallet 发起交易前，为用户在对应链上部署智能合约钱包
...
- **isEIP191Prefix** 是否使用标准 EIP191 前缀。默认值为 `false`, 使用 _UniPassPrefix_
