...
智能合约钱包并不是没有密钥，而是可以通过智能合约灵活地设置和管理钱包的密钥。

UniPass Contract 是一个通用的可以生成智能钱包的合约，主要管理支持的密钥类型以及密钥和账户权限之间的逻辑。而基于 UniPass Contract 生成的智能钱包，可以视为是 UniPass Contract 的实例。
